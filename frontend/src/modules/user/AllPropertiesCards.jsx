...
const AllPropertiesCards = ({ loggedIn }) => {
   ...
   const toggleLike = async (propertyId) => {
      try {
         ...
         let newLikedProperties;

         if (isLiked) {
            newLikedProperties = likedProperties.filter((id) => id !== propertyId);
         } else {
            newLikedProperties = [...likedProperties, propertyId];
         }

         setLikedProperties(newLikedProperties);

         const response = await axios.post(`http://localhost:8001/api/user/toggleLike`, { propertyId }, {
            headers: {
               Authorization: `Bearer ${localStorage.getItem('token')}`
            }
         });
                                 ...
                                 {property.isAvailable === "Available" ? (
                                    <>
                                       <div className='d-flex flex-wrap justify-content-between align-item-center'>
                                          <Button onClick={() => handleShow(property._id)} style={{ float: 'right' }} variant="outline-dark">Get Info</Button>
                                          <span
                                             className={`like-icon ${likedProperties.includes(property._id) ? 'liked' : ''}`}
                                             onClick={() => toggleLike(property._id)}
                                          >
                                             <FavoriteBorderIcon style={{ cursor: "pointer", fontSize: 30 }} />
                                             <span>{likedProperties.includes(property._id) ? property.likes : 0}</span>
                                          </span>
                                       </div>
